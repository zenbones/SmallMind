...
/**
 * Owns the single connection shared by all of a transport's {@link MessageRouter} instances, and
 * supervises its recovery. Two properties matter, and both were absent from the per-router recovery
 * this replaces.
 *
 * <p>First, every router of a transport declares the same per-instance queue names, and an exclusive
 * queue belongs to a connection rather than to a channel, so the routers must share one connection for
 * those queues to be declared exclusively at all.
 *
 * <p>Second, a recovery attempt that fails must schedule the next attempt. The previous implementation
 * re-declared from the channel's own shutdown listener, so a failure to open a connection left no
 * listener registered anywhere and recovery simply ended, leaving a worker silently consuming nothing
 * until its pod was restarted.
 *
 * <p>Recovery never runs on the client's shutdown dispatch thread. Listeners enqueue, and the work
 * happens on this manager's own scheduled executor with capped exponential backoff, retrying for as
 * long as the transport is open.
 */
public class RabbitMQConnectionManager {
  ...
  /**
   * Creates a manager for one transport's connection.
   *
   * @param connector     source of new connections, and of the retry tuning.
   * @param transportName short name used in thread names and log messages to identify the transport.
   */
  ...
  /**
   * Describes a shutdown signal in the broker's own words, verbatim. The conditions that matter here
   * are all diagnosable on sight from the reply text, but only if nobody paraphrases them.
   *
   * @param shutdownSignalException the signal to describe, possibly null.
   * @return a description suitable for a log line, naming the level the failure occurred at.
   */
  ...
  /**
   * Describes any throwable, preferring the broker's own words when it carries a shutdown signal.
   *
  ...
  private static String describeThrowable (Throwable throwable) {
    ...
    if ((shutdownSignalException = ShutDownSignalUtility.asShutdownSignal(throwable)) != null) {
  ...
  /**
   * Registers a router to be rebuilt whenever the connection is replaced. Called from the router's own
   * constructor.
   *
   * @param messageRouter the router to manage.
   */
  ...
  /**
   * Opens the connection and builds every registered router's channel.
   *
   * <p>Fails fast. If the broker is unreachable at startup the transport should not come up pretending
   * otherwise - recovery is for connections that were once established.
   *
   * @throws IOException      if the connection cannot be opened, or a router cannot be built.
   * @throws TimeoutException if opening the connection times out.
   */
  public void start ()
  ...
  /**
   * Returns the current connection generation, incremented every time the connection is replaced and
   * used to discard failures reported against a connection that has already been superseded.
   *
   * @return the current generation stamp.
   */
  ...
  /**
   * Returns whether this manager has been closed, after which nothing is rebuilt or retried.
   *
  ...
  /**
   * Returns the connection currently owned by this manager.
   *
   * @return the current connection, or null while one is being established.
   */
  ...
  /**
   * Reports the loss of one router's channel. A channel level failure rebuilds that channel alone; a
   * connection level failure reported through a channel replaces the connection, since nothing on it
   * is salvageable.
   *
   * @param messageRouter           the router whose channel failed.
   * @param failedGeneration        the generation that channel belonged to; a stale value is ignored.
   * @param shutdownSignalException the broker's reason, or null if none was carried.
   */
  ...
  /**
   * Reports the loss of the connection itself, scheduling its replacement.
   *
   * @param failedGeneration        the generation that failed; a stale value is ignored.
   * @param shutdownSignalException the broker's reason, or null if none was carried.
   */
  ...
  /*
   * The escalate flag separates two failures that look alike and are not. A channel that keeps
   ...
   */
  private void scheduleChannelRebuild (MessageRouter messageRouter, int failedGeneration, String description, boolean escalate) {

    if (failedGeneration == generation.get()) {

      int attempt = messageRouter.incrementChannelFailureCount();

      if (escalate && (attempt >= connector.getChannelFailureEscalationCount())) {
        ...
        logFailure(attempt, escalate ? "channel" : "binding", description);
        schedule(attempt, () -> {
          ...
          synchronized (connectionLock) {
            if ((!closed.get()) && (failedGeneration == generation.get())) {
              try {
                ...
                onChannelFailure(messageRouter, failedGeneration, ShutDownSignalUtility.asShutdownSignal(exception));
              }
            }
          }
        });
    ...
    synchronized (connectionLock) {
      ...
      for (MessageRouter messageRouter : messageRouterList) {
        ...
        try {
          ...
          ShutdownSignalException shutdownSignalException = ShutDownSignalUtility.asShutdownSignal(exception);
          ...
          scheduleChannelRebuild(messageRouter, replacementGeneration, describeThrowable(exception), true);
        }
      }
    }
  }
  ...
  /**
   * Returns whether this transport is actually able to carry traffic: an open connection, and every
   * registered router with an open channel, its queues bound, and its consumers installed when they
   * are required. This is the question a liveness probe should be asking.
   *
   * @return true if the transport is carrying, or able to carry, traffic.
   */
  ...
  /**
   * Returns a description of the connection, its generation, the last failure seen, and each router's
   * channel, binding and consumer state.
   *
   * @return diagnostic description intended for an operator reading it during an incident.
   */
  ...
  /**
   * Closes the manager, stopping all retries, marking every router closed, and closing the connection.
   */
  public void close () {
