...
public class InvocationWorker extends Worker<RabbitMQMessage> {
  ...
  public void engageWork (final RabbitMQMessage message)
    ...
    Instrument.with(Identifier.instance(InvocationWorker.class, "invoke"), LazyBuilder.instance(SpeedometerBuilder::new), new Tag("service", invocationSignal.getAddress().getService()), new Tag("method", invocationSignal.getAddress().getFunction().getName())).on(
