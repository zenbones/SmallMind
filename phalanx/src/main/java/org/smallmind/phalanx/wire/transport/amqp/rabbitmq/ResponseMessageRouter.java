...
public class ResponseMessageRouter extends MessageRouter {
  ...
  public void bindQueues ()
    ...
    for (RoutedQueue routedQueue : getRoutedQueues()) {
      if (declareAndBind(routedQueue.queueName(), routedQueue.ephemeral(), routedQueue.declaration())) {
        boundQueueSet.add(routedQueue.queueName());
      } else {
        fullyBound = false;
      }
    }
  ...
  public String publish (String callerId, String correlationId, boolean error, String nativeType, Object result)
    ...
    send("response-" + callerId, getResponseExchangeName(), rabbitMQMessage.properties(), rabbitMQMessage.body());

    return rabbitMQMessage.properties().getMessageId();
  }
