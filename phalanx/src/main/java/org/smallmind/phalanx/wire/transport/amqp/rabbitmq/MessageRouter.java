...
public abstract class MessageRouter {
  ...
  protected boolean declareAndBind (String queueName, boolean ephemeral, ChannelOperation channelOperation)
    ...
    try (Channel declarationChannel = connection.createChannel()) {
      ...
      if (((shutdownSignalException = ShutDownSignalUtility.asShutdownSignal(exception)) == null) || shutdownSignalException.isHardError()) {
    ...
    if (!closed.get()) {
      ...
      do {
        ...
        try {
          ...
          connectionManager.onChannelFailure(this, generation, ShutDownSignalUtility.asShutdownSignal(alreadyClosedException));
          try {
            TimeUnit.MILLISECONDS.sleep(50);
          } catch (InterruptedException interruptedException) {
            Thread.currentThread().interrupt();
