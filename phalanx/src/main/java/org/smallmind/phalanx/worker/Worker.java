      ...
      while (!stopped.get()) {
        try {
          ...
          if ((transfer = workQueue.poll(1, TimeUnit.SECONDS)) != null) {
            Instrument.with(Identifier.instance(Worker.class, "idle"), LazyBuilder.instance(SpeedometerBuilder::new)).update(System.nanoTime() - idleStart, TimeUnit.NANOSECONDS);
