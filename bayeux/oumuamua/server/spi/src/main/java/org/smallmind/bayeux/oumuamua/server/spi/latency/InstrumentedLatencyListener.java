        ...
        if (((latencyValue = extValue.get("latency")) != null) && ValueType.OBJECT.equals(latencyValue.getType())) {
          ...
          if (((timestampValue = ((ObjectValue<V>)latencyValue).get("timestamp")) != null) && ValueType.NUMBER.equals(timestampValue.getType())) {
            ...
            if (timeInTransit >= 0) {
              ...
              Instrument.with(InstrumentedLatencyListener.class, LazyBuilder.instance(HistogramBuilder::new), new Tag("host", hostName), new Tag("delivery", Boolean.toString(PacketType.DELIVERY.equals(outgoingPacket.getPacketType()))), new Tag("remote", Boolean.toString(isRemote(extValue)))).update(timeInTransit + 1);
            }
          }
        }
      }
