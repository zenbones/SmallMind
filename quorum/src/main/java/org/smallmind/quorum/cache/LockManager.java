...
@InstrumentedClass
public abstract class LockManager<K, V> implements ExternallyLockedCache<K, V> {
   ...
   private long externalLockTimeout;
   ...
   public LockManager (ReentrantLock[] stripeLocks, long externalLockTimeout)
   ...
   public KeyLock lock (KeyLock keyLock, K key) {
      ...
      try {
         if ((keyCondition = keyConditionMap.get(key)) == null) {
            keyConditionMap.put(key, keyCondition = new KeyCondition<K>(this, key, externalLockTimeout));
   ...
   private static class KeyCondition<K> implements Delayed {
      ...
      private LockManager<K, ?> lockingCache;
      private K key;
      ...
      public KeyCondition (LockManager<K, ?> lockingCache, K key, long externalLockTimeout) {
