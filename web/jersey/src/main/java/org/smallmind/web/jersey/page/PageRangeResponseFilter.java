    ...
    if ((resourceInfo.getResourceMethod() != null) && ((resourceInfo.getResourceMethod().getAnnotation(PageRange.class)) != null) && ((entityClass = responseContext.getEntityClass()) != null) && Page.class.isAssignableFrom(entityClass)) {
      ...
      if (page.getResultSize() == 0) {
        if (page.getFirstResult() == 0) {
          ...
          responseContext.getHeaders().add("Content-Range", "records */" + ((page.getTotalResults() < 0) ? "?" : String.valueOf(page.getTotalResults())));
          responseContext.setStatus(HTTP_DATA_OUT_OF_RANGE);
        }
      } else {
        responseContext.getHeaders().add("Content-Range", new StringBuilder("records ").append(page.getFirstResult()).append('-').append(page.getFirstResult() + page.getResultSize() - 1).append('/').append((page.getTotalResults() < 0) ? "?" : String.valueOf(page.getTotalResults())));
