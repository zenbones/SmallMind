...
public class ProxyResponseConsumer implements HttpAsyncResponseConsumer<ProxyHttpExchange> {
  ...
  public void responseReceived (final HttpResponse response) {
    ...
    synchronized (this.httpExchange) {
      LoggerManager.getLogger(ProxyRequestConsumer.class).trace("[proxy<-origin] %s %s", this.httpExchange.getId(), response.getStatusLine());
      ...
      if (responseTrigger != null && !responseTrigger.isCompleted()) {
        LoggerManager.getLogger(ProxyRequestConsumer.class).trace("[client<-proxy] %s response triggered", this.httpExchange.getId());
  ...
  public void consumeContent (final ContentDecoder decoder, final IOControl ioctrl) throws IOException {
    ...
    synchronized (this.httpExchange) {
      ...
      LoggerManager.getLogger(ProxyRequestConsumer.class).trace("[proxy<-origin] %s %d bytes read", this.httpExchange.getId(), n);
      if (decoder.isCompleted()) {
        LoggerManager.getLogger(ProxyRequestConsumer.class).trace("[proxy<-origin] %s content fully read", this.httpExchange.getId());
      }
      // If the buffer is full, suspend origin input until there is free
      // space in the buffer
      if (!buf.hasRemaining()) {
        ioctrl.suspendInput();
        LoggerManager.getLogger(ProxyRequestConsumer.class).trace("[proxy<-origin] %s suspend origin input", this.httpExchange.getId());
      }
      ...
      if (buf.position() > 0) {
        if (this.httpExchange.getClientIOControl() != null) {
          ...
          LoggerManager.getLogger(ProxyRequestConsumer.class).trace("[proxy<-origin] %s request client output", this.httpExchange.getId());
        }
      }
    }
  }
  ...
  public void responseCompleted (final HttpContext context) {
    ...
    synchronized (this.httpExchange) {
      ...
      this.completed = true;
      LoggerManager.getLogger(ProxyRequestConsumer.class).trace("[proxy<-origin] %s response completed", this.httpExchange.getId());
  ...
  public void failed (final Exception exception) {
    ...
    synchronized (this.httpExchange) {
      ...
      this.completed = true;
      this.httpExchange.setException(exception);
      HttpAsyncExchange responseTrigger = this.httpExchange.getResponseTrigger();
      if (responseTrigger != null && !responseTrigger.isCompleted()) {
