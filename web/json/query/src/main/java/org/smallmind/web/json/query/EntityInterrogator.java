...
public class EntityInterrogator {
  ...
  private static <E> boolean matchesCriterion (E entity, WhereCriterion criterion) {
    ...
    return switch (criterion.getCriterionType()) {
      ...
      case FIELD -> {
        try {
          yield ((WhereField)criterion).getOperator().isTrue(((WhereField)criterion).getOperand(), WhereOperand.fromObject(BeanReflector.get(entity, ((WhereField)criterion).getName())));
    ...
    public int compare (E firstEntity, E secondEntity) {
      ...
      for (SortField sortField : sortFields) {
        try {
          ...
          WhereOperand<?> firstOperand = WhereOperand.fromObject(BeanReflector.get(firstEntity, sortField.getName()));
          WhereOperand<?> secondOperand = WhereOperand.fromObject(BeanReflector.get(secondEntity, sortField.getName()));
