...
public class WebServiceInstallerTest {
  ...
  public void testConstructorStoresPathAndService () {

    Object service = new Object();
    WebServiceInstaller installer = new WebServiceInstaller("/endpoint", service);

    Assert.assertEquals(installer.getPath(), "/endpoint");
  ...
  public void testAccessorRoundTrip () {
    ...
    WebServiceInstaller installer = new WebServiceInstaller("/endpoint", firstService);

    installer.setPath("/other");
    installer.setService(secondService);
    installer.setAsyncSupported(Boolean.TRUE);

    Assert.assertEquals(installer.getPath(), "/other");
    Assert.assertSame(installer.getService(), secondService);
