...
public class GrizzlyInitializingBeanSSLBootTest {
  ...
  public void testMutualTlsRequiresClientCertificate ()
    throws Exception {

    String keyStoreResource = stageSelfSignedKeyStore();

    if (keyStoreResource == null) {
      throw new SkipException("Could not generate or stage a self-signed keystore via keytool; skipping mutual-TLS enforcement scenario.");
    }

    java.nio.file.Path stagedKeyStoreFile = java.nio.file.Paths.get(GrizzlyInitializingBeanSSLBootTest.class.getResource("/grizzly-https-test-keystore.jks").toURI());
    int httpsPort = freePort();
    ...
    try (GenericApplicationContext applicationContext = new GenericApplicationContext()) {
      ...
      applicationContext.registerBean("grizzlyServer", GrizzlyInitializingBean.class, () -> {
        ...
        sslInfo.setKeySecureStore(keySecureStore);
        sslInfo.setTrustSecureStore(trustSecureStore);
        // requireClientAuth must drive the SSLEngineConfigurator's needClientAuth so the server demands a client
        // certificate at the handshake; without this wiring mutual TLS would silently not be enforced.
