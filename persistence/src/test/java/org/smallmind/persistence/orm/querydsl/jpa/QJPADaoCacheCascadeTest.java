...
/**
 * Unit tests for the cache-cascade ({@code vectoredDao != null}) branches of {@link QJPADao}'s
 * {@code get}/{@code persist}/{@code delete} methods. The {@link JPAProxySession} and its native
 * {@link EntityManager} are Mockito mocks, so no database is involved; the assertions verify read-through
 ...
 */
@Test(groups = "unit")
public class QJPADaoCacheCascadeTest {
  ...
  public void testDeleteOfDetachedDurableFindsThenRemovesAndEvicts () {
    ...
    Widget persistedState = new Widget(7L);

    Mockito.when(entityManager.contains(durable)).thenReturn(false);
    Mockito.when(entityManager.find(Widget.class, 7L)).thenReturn(persistedState);

    dao.delete(durable);

    Mockito.verify(entityManager).remove(persistedState);
    Mockito.verify(vectoredDao).delete(Widget.class, durable);
  }

  public static class Widget extends JPADurable<Long, Widget> {
  ...
  private static class WidgetDao extends QJPADao<Long, Widget> {
