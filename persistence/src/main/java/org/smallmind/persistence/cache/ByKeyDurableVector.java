...
public class ByKeyDurableVector<I extends Serializable & Comparable<I>, D extends Durable<I>> extends DurableVector<I, D> {
  ...
  private DurableKeyConcurrentList<I, D> elements;

  public ByKeyDurableVector (Class<D> durableClass, ConcurrentList<D> durables, Comparator<D> comparator, int maxSize, long timeToLive, boolean ordered) {

    super(comparator, maxSize, timeToLive, ordered);

    ConcurrentList<DurableKey<I, D>> keyList = new ConcurrentList<DurableKey<I, D>>();
    int index = 0;
    ...
    elements = new DurableKeyConcurrentList<I, D>(durableClass, keyList);
  }

  private ByKeyDurableVector (DurableKeyConcurrentList<I, D> elements, Comparator<D> comparator, int maxSize, long timeToLive, boolean ordered) {
  ...
  public DurableVector<I, D> copy () {
    ...
    return new ByKeyDurableVector<I, D>(new DurableKeyConcurrentList<I, D>(elements), getComparator(), getMaxSize(), getTimeToLive(), isOrdered());
  }
