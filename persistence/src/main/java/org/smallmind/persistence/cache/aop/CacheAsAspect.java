    ...
    try {
      ...
      if (cacheAs.singular()) {
        ...
        if ((durable = (Durable)thisJoinPoint.proceed()) != null) {
          if (vectoredDao != null) {
            vectoredDao.readLock(ormDao.getManagedClass(), durable.getId());
            try {
              ...
              vectoredDao.readUnlock(ormDao.getManagedClass(), durable.getId());
            }
          }
        }
        ...
        if ((iterable = (Iterable)thisJoinPoint.proceed()) != null) {
          if (vectoredDao != null) {
            ...
            LinkedList<Durable> everyDurable;
            Comparable[] ids;
            int index = 0;

            everyDurable = new LinkedList<Durable>();
            for (Object obj : iterable) {
              everyDurable.add((Durable)obj);
            }

            ids = new Comparable[everyDurable.size()];
            for (Durable durable : everyDurable) {
              ids[index++] = durable.getId();
            }

            vectoredDao.readLock(ormDao.getManagedClass(), ids);
            try {

              DurableVector vector = vectoredDao.createVector(vectorKey, everyDurable, cacheAs.comparator().equals(Comparator.class) ? null : cacheAs.comparator().newInstance(), cacheAs.max(), getTimeToLive(cacheAs), cacheAs.ordered());

              return vectoredDao.persistVector(vectorKey, vector).asList();
            }
            finally {
              vectoredDao.readUnlock(ormDao.getManagedClass(), ids);
            }
          }
        }
