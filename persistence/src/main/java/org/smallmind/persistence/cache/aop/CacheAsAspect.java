...
public class CacheAsAspect {
  ...
  @Around(value = "execution(@CacheAs * * (..)) && @annotation(cacheAs) && this(durableDao)", argNames = "thisJoinPoint, cacheAs, durableDao")
  public Object aroundCacheAsMethod (ProceedingJoinPoint thisJoinPoint, CacheAs cacheAs, VectorAwareDurableDao durableDao)
    throws Throwable {
    ...
    try {
      ...
      if (durableDao.getManagedClass().equals(methodSignature.getReturnType())) {
        if (cacheAs.ordered()) {
        ...
        if ((vectoredDao = durableDao.getVectoredDao()) == null) {
          ...
          vectorKey = new VectorKey(VectorIndices.getVectorIndexes(cacheAs.value(), thisJoinPoint), durableDao.getManagedClass(), Classifications.get(CacheAs.class, thisJoinPoint, cacheAs.value()));
        ...
        if ((!((returnType = (executedMethod = methodSignature.getMethod()).getGenericReturnType()) instanceof ParameterizedType)) || (!durableDao.getManagedClass().equals(((ParameterizedType)returnType).getActualTypeArguments()[0]))) {
          throw new CacheAutomationError("Methods annotated with @CacheAs which return an Iterable type must be parameterized to <? extends Iterable<%s>>", durableDao.getManagedClass().getSimpleName());
        }
        ...
        throw new CacheAutomationError("Methods annotated with @CacheAs must either return their managed type(%s), or an Iterable parameterized to their managed type <? extends Iterable<%s>>", durableDao.getManagedClass().getSimpleName(), durableDao.getManagedClass().getSimpleName());
      }
    }
      ...
      if (timingEnabled) {
        ...
        InstrumentationManager.instrumentWithChronometer(PersistenceManager.getPersistence(), stop - start, TimeUnit.MILLISECONDS, new MetricProperty("durable", durableDao.getManagedClass().getSimpleName()), new MetricProperty("method", executedMethod.getName()), new MetricProperty("source", metricSource));
      }
    }
  }
