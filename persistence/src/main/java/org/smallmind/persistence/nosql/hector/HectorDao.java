...
public abstract class HectorDao<W extends Serializable & Comparable<W>, I extends Serializable & Comparable<I>, D extends Durable<I>> extends NoSqlDao<W, I, D> {
  ...
  private static final DateTranslator DATE_TRANSLATOR = new DateTranslator();
  private static final EnumTranslator ENUM_TRANSLATOR = new EnumTranslator();
  ...
  private static final BooleanTranslator BOOOELAN_TRANSLATOR = new BooleanTranslator();
  private static final CharacterTranslator CHARACTER_TRANSLATOR = new CharacterTranslator();
  ...
  public List<D> get (Class<Durable<W>> parentClass, W parentId, Class<D> durableClass) {
    ...
    if ((hectorResult = hectorTemplate.queryColumns(new Composite(parentClass.getSimpleName(), parentId))).hasResults()) {
      ...
      for (Composite columnName : hectorResult.getColumnNames()) {
        ...
        nonKeyType = nonKeyField.getType();

        naturalKeyIndex = 0;
        for (Field naturalKeyField : naturalKeyFields) {
          naturalKeyValues[naturalKeyIndex] = getTranslator(naturalKeyField.getType(), naturalKeyField.getName()).getFieldValue(naturalKeyField.getType(), naturalKeyIndex++, columnName);
        }
  ...
  private NaturalKeyTranslator<?> getTranslator (Class<?> fieldType, String fieldName) {
  ...
  public List<D> persist (Class<Durable<W>> parentClass, W parentId, Class<D> durableClass, List<D> durables) {
    ...
    if ((durables != null) && (!durables.isEmpty())) {
      ...
      ColumnFamilyUpdater<Composite, Composite> updater = hectorTemplate.createUpdater(new Composite(parentClass.getSimpleName(), parentId));
      WideVectoredDao<W, I, D> wideVectoredDao;
