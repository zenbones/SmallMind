...
public class ThrongEntityMultiplexer<T> extends ThrongPropertiesMultiplexer<T> {

  private final String collection;
  private ThrongProperty idProperty;

  public ThrongEntityMultiplexer (Class<T> entityClass, Entity entityAnnotation, CodecRegistry codecRegistry, HashMap<Class<?>, Codec<?>> embeddedReferenceMap, boolean storeNulls)
    ...
    super(entityClass, entityAnnotation.polymorphic(), codecRegistry, embeddedReferenceMap, storeNulls);
    ...
    for (FieldAccessor fieldAccessor : FieldUtility.getFieldAccessors(entityClass)) {
      ...
      if ((idAnnotation = fieldAccessor.getField().getAnnotation(Id.class)) != null) {
        if (fieldAccessor.getField().getAnnotation(Property.class) != null) {
