  ...
  public static Codec<?> generateEmbeddedCodec (Class<?> embeddedType, Embedded embedded, CodecRegistry codecRegistry, HashMap<Class<?>, Codec<?>> embeddedReferenceMap, boolean storeNulls)
    ...
    if ((codec = embeddedReferenceMap.get(embeddedType)) == null) {
      ...
      if ((polymorphic = embedded.value()).value().length > 0) {
        embeddedReferenceMap.put(embeddedType, codec = new ThrongPolymorphicEmbeddedCodec<>(new ThrongPolymorphicMultiplexer<>(embeddedType, polymorphic, codecRegistry, embeddedReferenceMap, storeNulls)));
      } else {
