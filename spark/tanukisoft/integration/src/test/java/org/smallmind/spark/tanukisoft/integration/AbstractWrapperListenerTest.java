...
// The surfaces here reach WrapperManager only after the guard has already failed (empty args) or not at all (stop):
// the bootstrap argument guard and the stop() exit-code translation. The start()/controlEvent()/main() paths that do
// drive WrapperManager are covered in AbstractWrapperListenerLifecycleTest against a shadowed WrapperManager double.
...
public class AbstractWrapperListenerTest {
  ...
  private static class RecordingListener extends AbstractWrapperListener {
    ...
    private RecordingListener (boolean shutdownThrows) {

      this.shutdownThrows = shutdownThrows;
    }
