...
public class JarWithinJarName {

  private String outerJarName;
  private String outerEntryName;
  private String innerEntryName;

  public JarWithinJarName (URL url) {

    int doubleBangPos = url.getPath().indexOf("!!/");
    int singBangPos = url.getPath().indexOf("!/", doubleBangPos + 3);

    outerJarName = url.getPath().substring(0, doubleBangPos);
    outerEntryName = url.getPath().substring(doubleBangPos + 3, singBangPos);
    innerEntryName = url.getPath().substring(singBangPos + 2);
  }

  public String getOuterJarName () {

    return outerJarName;
  }

  public String getOuterEntryName () {

    return outerEntryName;
  }
