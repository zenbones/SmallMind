...
The default value is the ImpliedNamingStrategy (see <<claxon-configuration-implied-naming-strategy>> below)
...
The concatenation of the meter's root name with each quantity's name will form the _full_ name of that quantity passed to each of the registry's bound emitters (see <<claxon-emitters>> below).
...
The ObviousNamingStrategy holds a Set of DotNotation (see <<claxon-dot-notation>> below) instances which it attempts to match against the names of the caller classes submitted with meter registrations.
...
The ImpliedNamingStrategy holds a Map of DotNotation (see <<claxon-dot-notation>> below) to String entries.
This strategy attempts to match the DotNotation keys against the names of the caller classes submitted with meter registrations.
If no match can be found, then no meter will be registered, and no metrics will be emitted.
