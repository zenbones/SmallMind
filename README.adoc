...
* *Map<DotNotation, String>* (_prefixMap_) - A mapping from DotNotation instances (see <<claxon-dot-notation>> below) to meter names. When a meter is registered (see <<claxon-registry-register>> above) the class name of the `Class<?> caller` parameter is matched against all known DotNotation instances. If a DotNotation can't be found that matches the class name, no meter will be registered, and no metrics will be emitted. If one or more matches are found, the strongest match, which is the one matching the most dot notated segments in the name, will determine the root name for all metric quantities produced by that meter. The _full_ name of each quantity is the concatenation of the root name (which is the mapped String value of the winning DotNotation key) _plus_ the name of the quantity produced by the meter. Exactly how that name is formed is dependent upon the logic of each Emitter (for the specifics see the <<claxon-emitters>> section below).
+
[TIP]
If you wanted to emit all metrics with a single root name you could add a single _prefixMap_ entry like `put(new DotNotation("*"), "my.metrics")`. This may seem like a crowded name, but the name of any quantity should serve only to broadly categorize its value. The real namespace comes from the set of registered tags. It is, therefore, very important that the set of Tags registered for each meter differentiate it from all other meters with the same root name.
...
. Call the `initializeInstrumentation()` method of the configured Registry instance.
+
[TIP]
...
The prerequisites taken care of, Instrument functionality is accessed through its `with()` method. This method takes the same caller, builder and tags parameters as the <<claxon-registry-register>> method discussed previously, but returns an instance of the Instrumentation (`org.smallmind.claxon.registry.Instrumentation`) interface. Through this interface you can...
...
* `as()` to set the default time unit of the Instrumentation instance.
* `on()` to wrap a block of code in timing metrics (passed as a Lambda with or without a return value).
+
[NOTE]
====
[source, java]
