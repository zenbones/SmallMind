...
public abstract class HistogramInstrument extends Instrument<Histogram> {
  ...
  public HistogramInstrument (MetricConfigurationProvider provider, MetricProperty... properties) {
  ...
  public abstract void withHistogram (AtomicLong valueRef)
    throws Exception;

  @Override
  public final void with (Histogram histogram)
    throws Exception {

    AtomicLong valueRef = new AtomicLong(1);

    withHistogram(valueRef);

    if (histogram != null) {
      histogram.update(valueRef.get());
    }
  }
}
