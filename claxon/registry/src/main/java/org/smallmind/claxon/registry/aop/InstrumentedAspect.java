...
public class InstrumentedAspect {
  ...
  public Object aroundInstrumentedMethod (ProceedingJoinPoint thisJoinPoint, Instrumented instrumented)
    ...
    Identifier identifier = instrumented.id().domain().isEmpty() ? Identifier.instance(instrumented.id().name()) : Identifier.instance(instrumented.id().domain(), instrumented.id().name());
    int index = 0;
