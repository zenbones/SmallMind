...
public class WishfulPrometheusCollector extends PullCollector<String> {
  ...
  */
  ...
  public void record (String meterName, Tag[] tags, Quantity[] quantities) {
    ...
    StringBuilder identityBuilder = mangle(new StringBuilder(), meterName);
    ...
    if ((tags != null) && (tags.length > 0)) {
      ...
      for (Tag tag : tags) {
        ...
        mangle(labelBuilder, tag.getKey()).append("=\"").append(tag.getValue()).append('"');
        first = false;
      }
      labelBuilder.append("}");
    }
    ...
    for (Quantity quantity : quantities) {
      ...
      mangle(rowBuilder.append(':'), quantity.getName());
  ...
  private StringBuilder mangle (StringBuilder mangledBuilder, String original) {
